...
def run_ai_qa(sheet_context, img_path, lang_name="", target_timezone="未指定"):
    ...
    prompt = f"""
    你是一名商業數位行銷內容的專業 QA 核對人員。請比對宣傳頁面截圖（當前對稿語系：【{lang_name}】）與企劃規格檔案：
    ...
    3. 🔍 [數字日期格式與語系等價律]：
       - 數字日期格式（如 "09/05" 或 "9/5"）與各語系日期文字（葡文 "5 de setembro"、英文 "Sep 5"、中文 "9月5日"）完全相等！
       - 在英文版或國際版 Banner 上，採用 "09/05 01:00 AM – 09/10 00:59 AM" 這種全數字格式是絕對正確且標準的作法。
       - **絕對不要要求英文或非葡文 Banner 上必須顯示葡文單字（如 "de setembro"）**。
       - 只要 Banner 上的月份（9月）、日期（5日與10日）以及時間（【{expected_start}】與【{expected_end}】）完全一致，【必須判定為 ✅ 通過】！
